...
class AuthViewModel:ViewModel() {
    ...
    fun signup(username:String,email:String,password:String,confirmpassword:String,navController: NavController,number: String,context:Context){
        if (username.isBlank() || email.isBlank() || password.isBlank() || confirmpassword.isBlank() || number.isBlank()){
        ...
        auth.createUserWithEmailAndPassword(email,password).addOnCompleteListener{
            ...
            if (task.isSuccessful){
                ...
                val user = UserModel(username=username,email=email,number=number,userId=userId)
